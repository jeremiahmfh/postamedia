...
app.get('/test-firebase', async (req, res) => {
  try {
    ...
    console.log('Firebase FieldValue initialized:', !!FieldValue);
    ...
    if (!db) {
      return res.json({ 
        ...
        fieldValueInitialized: !!FieldValue
      });
    }
  ...
  try {
    ...
    const yocoResponse = await axios.post(
      `${YOCO_CHECKOUT_URL}`,
      {
        ...
        success_url: callbackUrl ? `${callbackUrl}?success=true&amount=${amount}` : undefined,
    ...
    if (withdrawalData.yocoPayoutId) {
      try {
        ...
        await withdrawalDoc.ref.update({
          ...
          completedAt: payoutData.status === 'succeeded' ? FieldValue.serverTimestamp() : null
        });
